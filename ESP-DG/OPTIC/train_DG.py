    ...
    def build_model(self):
        if self.model_type == 'Res_Unet':
            self.model = ResUnet(resnet=self.backbone, num_classes=self.out_ch, pretrained=True,
                                 n_domains=self.n_domains, domain_discriminator_flag=self.domain_discriminator_flag, grl=self.grl, lambd=self.lambd,
                                 drop_percent=self.drop_percent, filter_WRS_flag=self.filter_WRS_flag, recover_flag=self.recover_flag).to(self.device)
        else:
