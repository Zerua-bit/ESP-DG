    ...
    def build_model(self):
        if self.model_type == 'Res_Unet':
            self.model = ResUnet(resnet=self.backbone, num_classes=self.out_ch, pretrained=True,
                                 n_domains=self.n_domains, domain_discriminator_flag=0, grl=self.grl, lambd=self.lambd,
                                 drop_percent=self.drop_percent, filter_WRS_flag=self.filter_WRS_flag, recover_flag=self.recover_flag
                                 )
        else:
    ...
    def test(self):
        ...
        with torch.no_grad():
            for batch, data in enumerate(self.test_loader):
                ...
                seg_logit, domain_logit, self_distillation_loss  = self.model(x)
