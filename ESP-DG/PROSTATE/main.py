...
def main(config):
    ...
    sr_img_list, sr_label_list, sr_domain_label_list = convert_labeled_list(config.dataset_root, source_csv)
    ...
    if config.mode == 'train_kfold':
        ...
        source_dataset = PROSTATE_dataset(config.dataset_root, sr_img_list, sr_label_list, sr_domain_label_list,
                                          config.image_size, config.batch_size, img_normalize=False)
        ...
        ts_img_list, ts_label_list, ts_domain_label_list = convert_labeled_list(config.dataset_root, target_test_csv)

        target_valid_dataset = PROSTATE_dataset(config.dataset_root, ts_img_list, ts_label_list, ts_domain_label_list,
                                                config.image_size, img_normalize=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_domains', type=int, default=5, help="numbers of source domains.")  ##############
    ...
    parser.add_argument("--drop_percent", default=0.3, type=float, help="percent of dropped filters")
