...
class PROSTATE_dataset(data.Dataset):
    def __init__(self, root, img_list, label_list, domain_label_list, target_size=384, batch_size=None, img_normalize=True):
        super().__init__()
        self.root = root
        self.img_list = img_list
        self.label_list = label_list
        self.domain_label_list = domain_label_list
        self.target_size = (target_size, target_size)
        self.img_normalize = img_normalize
        self.image_pool, self.label_pool, self.name_pool, self.domain_label_pool = [], [], [], []
        ...
        if batch_size is not None:
            ...
            self.domain_label_pool = self.domain_label_pool * scale
    ...
    def __getitem__(self, item):
        ...
        img_domain_label, slice = self.domain_label_pool[item]
        ...
        return img_npy, label_npy, self.name_pool[item], img_domain_label
    ...
    def _read_img_into_memory(self):
        ...
        for index in range(img_num):
            ...
            # 获取当前图像对应的域标签
            domain_label = self.domain_label_list[index]
            ...
            for slice in range(img_npy.shape[0]):
                if label_npy[slice, :, :].max() > 0:
                    ...
                    self.domain_label_pool.append((domain_label, slice))
